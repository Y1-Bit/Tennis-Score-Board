...
class Match:
    ...
    @classmethod
    def create(cls, player1: Player, player2: Player) -> "Match":
        return cls(player1=player1, player2=player2)
        ...
        if self._is_game_won(player):
            ...
            if self._is_set_won(player, current_set):
                if self._is_match_won(player):
                    self.winner_id = (
                        self.player1.id if player == "player1" else self.player2.id
                    )
                else:
                    self._start_new_set()
    ...
    def _is_match_won(self, player: str) -> bool:
        ...
        sets_won = sum(
            1
            for set_name in ["set1", "set2", "set3"]
            if getattr(getattr(self.score, set_name), player)
            > getattr(getattr(self.score, set_name), opponent)
        )
        return sets_won == 2
